...
%                 * (C_Z + C_Z_q * P.c * q / (2 * Va) + C_Z_delta_e * delta_e);
    Force(1) = -P.mass*P.gravity*st + w_ns * P.rho;
    Force(2) = P.mass*P.gravity*ct*sp + w_es * P.rho;
    
    force_torque_matrix = [P.k1, P.k1, P.k1, P.k1;
                           0, -P.l*P.k1, 0, P.l*P.k1;
                           P.l*P.k1, 0, -P.l*P.k1, 0;
                           -P.k2, P.k2, -P.k2, P.k2];

    f_torque = force_torque_matrix * [delta_f; delta_r; delta_b; delta_l];
    
    Force(3) = -f_torque(1) + P.mass*P.gravity*ct*cp - w_ds * P.rho;
    Torque(1) = f_torque(2);
    Torque(2) = f_torque(3);
    Torque(3) = f_torque(4);
